...
class WeatherCog(commands.Cog, name="Weather"):
    ...
    @weather_group.command(name="test", description="Test gửi thông báo thời tiết")
    @app_commands.checks.has_permissions(administrator=True)
    async def test(self, interaction: discord.Interaction):
        """Test gửi thông báo thời tiết với nội dung giống thông báo thật."""
        config = load_json(JSON_CONFIG).get(str(interaction.guild_id), {})
        weather_config = config.get("weather")
        ...
        if not locations:
            await interaction.response.send_message(
                "❌ Chưa có vị trí nào được thêm. Vui lòng dùng `/weather add`.",
                ephemeral=True
            )
            return
        ...
        if not channel_id:
            await interaction.response.send_message(
                "❌ Chưa cấu hình channel. Vui lòng dùng `/weather setup`.",
                ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"✅ Đã gửi thông báo test đến {channel.mention}",
            ephemeral=True
        )
